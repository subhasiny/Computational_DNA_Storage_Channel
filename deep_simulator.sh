...
function usage()
{
	...
	echo "                [-f filter_freq] [-s signal_std] [-P perfect] [-H home] [-Y coverage_storage][-d coverage_distribution] "
	echo "Options:"
	echo ""
	...
	echo ""
	echo "-Y coverage_storage  : This parameter is used to control the coverage value for DNA storage technology. [default=1] "
	echo ""
	echo "-d coverage_distribution  : This parameter is used to control the coverage distribution for DNA storage technology. [default=1] "
	echo ""
...
COVERAGE_STORAGE=1  #-> the coverage parameter, we simulate read based on coverage size
COVERAGE_DISTRIBUTION=1  #-> the coverage parameter, we simulate read based on coverage distribution
...
#------- parse arguments ---------------#
while getopts ":i:o:D:c:S:B:n:K:l:C:m:M:e:u:O:G:f:s:P:H:Y:d:" opt;
do
	case $opt in
	#-> required arguments
	i)
	...
	K)
		...
		;;
	Y)
		COVERAGE_STORAGE=$OPTARG
		;;
	d)
		COVERAGE_DISTRIBUTION=$OPTARG
		;;
	l)
		LEN_MEAN=$OPTARG
		;;
...
#DISTRIBUTION GENERATION FOR AMPLIFYING DATA
echo "hi"
#python2 $home/util/data_distribution_parametric.py -Y $COVERAGE_STORAGE

if [ $COVERAGE_DISTRIBUTION -eq 1 ]
then
    python2 $home/util/case2_normal_non_parametric.py -Y $COVERAGE_STORAGE
fi

if [ $COVERAGE_DISTRIBUTION -eq 2 ]
then
    python2 $home/util/case2_beta_non-parametric.py -Y $COVERAGE_STORAGE
fi

if [ $COVERAGE_DISTRIBUTION -eq 3 ]
then
    python2 $home/util/case2_log-gamma_non-parametric.py -Y $COVERAGE_STORAGE
fi

if [ $COVERAGE_DISTRIBUTION -eq 8 ]
then
   echo "hi"
fi
...
cd sampled_readtry_DeepSimu
cat sampled_read_* >> sampled_read.fasta
sed -i '/^$/d' sampled_read.fasta 
cd ..
#python2 $home/util/remove_space.py
